...
class UbicationCreateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) { super.onCreate(savedInstanceState); setContentView(R.layout.model_ubication_create); SessionNavigation.bindProfile(this); initViews(); initEvents(); loadNextRegister() }
    ...
    private fun save(){ val name=fieldName.text.toString().trim(); if(name.isEmpty()){ Toast.makeText(this,"Debes ingresar el nombre",Toast.LENGTH_SHORT).show(); return}; val data=UbicationModel(generatedRegister,name); collection.document(generatedRegister.toString()).set(data).addOnSuccessListener{ Toast.makeText(this,"Registro creado",Toast.LENGTH_SHORT).show(); finish() }.addOnFailureListener{ Toast.makeText(this,"Error: ${it.message}",Toast.LENGTH_LONG).show() } }
}
