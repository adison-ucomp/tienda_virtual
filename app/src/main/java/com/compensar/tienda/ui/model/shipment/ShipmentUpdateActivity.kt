...
class ShipmentUpdateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) { super.onCreate(savedInstanceState); setContentView(R.layout.model_shipment_update); SessionNavigation.bindProfile(this); register=intent.getLongExtra("register",0); initViews(); initEvents(); load() }
    ...
    private fun update(){ val name=fieldName.text.toString().trim(); if(name.isEmpty()){Toast.makeText(this,"Debes ingresar el nombre",Toast.LENGTH_SHORT).show();return}; collection.document(register.toString()).set(ShipmentModel(register,name)).addOnSuccessListener{Toast.makeText(this,"Estado actualizado",Toast.LENGTH_SHORT).show();finish()}.addOnFailureListener{Toast.makeText(this,"Error: ${it.message}",Toast.LENGTH_LONG).show()} }
}
