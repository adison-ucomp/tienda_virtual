...
class BuyerAddressActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState); enableEdgeToEdge(); setContentView(R.layout.buyer_address); applyWindowInsets(); initViews(); initEvents(); loadUbicationsThenAddresses()
    }
    ...
    private fun loadAddresses(){ val user=SessionManager.getRegister(this); addressList.removeAllViews(); if(user<=0){ addEmpty("Debes iniciar sesión para ver tus direcciones"); return }; db.collection("address").whereEqualTo("idUser", user).orderBy("register", Query.Direction.ASCENDING).get().addOnSuccessListener{ result -> addressList.removeAllViews(); val items=result.documents.mapNotNull{it.toObject(AddressModel::class.java)}; if(items.isEmpty()) addEmpty("No tienes direcciones registradas"); items.forEach{ addressList.addView(card(it)) } }.addOnFailureListener{ e -> addEmpty("Error cargando direcciones: ${e.message}") } }
